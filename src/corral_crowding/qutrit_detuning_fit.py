...
def compute_infidelity_parameters(detuning_list, lambdaq, eta, alpha, g3):
    ...
    q = destroy(2)
    q1 = tensor(q, qeye(2), qeye(2))  # First qutrit (Main interaction)
    q2 = tensor(qeye(2), q, qeye(2))  # Second qutrit (Main interaction)
    q3 = tensor(qeye(2), qeye(2), q)  # Spectator qutrit
    q1dag = q1.dag()
    q2dag = q2.dag()
    q3dag = q3.dag()
    ...
    qs = destroy(2)  # qutrit part of SNAIL system
    s = destroy(n_dim_snail)  # SNAIL oscillator
    qs1 = tensor(qs, qeye(2), qeye(n_dim_snail))  # First qutrit
    qs2 = tensor(qeye(2), qs, qeye(n_dim_snail))  # Second qutrit
